...
void Valu_top::prepareClone() const { contextp()->prepareClone(); }
void Valu_top::atClone() const {
...
VL_ATTR_COLD void Valu_top::trace(VerilatedVcdC* tfp, int levels, int options) {
    vl_fatal(__FILE__, __LINE__, __FILE__,"'Valu_top::trace()' called on model that was Verilated without --trace option");
}
